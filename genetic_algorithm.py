...
def init_individual(job_nums: int, job_orders: List[int], job_gpus: List[int]) -> Individual:
    """
    初始化个体的JOB顺序以及GPU数量信息。

    :param job_nums: JOB数量。
    :param job_orders: JOB可能的顺序。
...
def calculation_process(job_names: List[str], gpu_nums: int,
    ...
    for individual in group:
        ...
        # 计算当前个体的完成时间，它等于调度方案当中每个分组的最长完成时间之和:
        completion_time = 0
        for job_group in job_group_list:
            group_time = 0
            for job in job_group:
                # 如果出现不可能的GPU数量，则该分组的完成时间直接为无穷大，即舍弃当前调度方案:
...
def selection(group: List[Individual]) -> List[Individual]:
    """
    选择过程。该函数返回的是一个新列表，对其内容进行改动不会影响传入列表。
    ...
    # 获得等同种群个数的随机数组，根据二分查找从累加概率数组当中获取个体索引，最后从种群当中获取个体:
...
def cross_over(group: List[Individual]):
    ...
    for cross_couple in cross_couples:
        ...
        fgs = group[cross_couple[0]].gpus

        sos = group[cross_couple[1]].orders
        sgs = group[cross_couple[1]].gpus
...
def preferential_admission(origin_group: List[Individual], change_group: List[Individual]) -> List[Individual]:
    """
    择优过程。将原始种群和经过选择、交叉、变异后的种群合并，按照个体调度方案完成时间排序，选择出前一半的个体组成当前迭代的最后种群。
